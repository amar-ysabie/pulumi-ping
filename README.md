...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `pingone` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `amar-ysabie` with the `ORG` of your provider.
   - replace all instances of the `github.com/amar-ysabie/pulumi-pingone` repository with the `REPOSITORY` location
...
before:
  ...
  - make tfgen
builds:
- binary: pulumi-resource-pingone
  dir: provider
  goarch:
  - amd64
  - arm64
  ...
  ignore: []
  ldflags:
  - -X github.com/amar-ysabie/pulumi-pingone/provider/pkg/version.Version={{.Tag}}
  main: ./cmd/pulumi-resource-pingone/
  sort: asc
  use: git
release:
  disable: false
...
├── config.yaml
├── metadata.json
├── pulumi-pingone_darwin_amd64_v1
│   └── pulumi-resource-pingone
├── pulumi-pingone_darwin_arm64
│   └── pulumi-resource-pingone
├── pulumi-pingone_linux_amd64_v1
│   └── pulumi-resource-pingone
├── pulumi-pingone_linux_arm64
│   └── pulumi-resource-pingone
├── pulumi-pingone_windows_amd64_v1
│   └── pulumi-resource-pingone.exe
└── pulumi-pingone_windows_arm64
    └── pulumi-resource-pingone.exe
```
